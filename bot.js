...
client.on("message", message => {
 if (message.content === "-invite") {
  const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setFooter('.')
      .addField('شكرا لك لاستخدامك ل بروميوم بوت', `https://discordapp.com/api/oauth2/authorize?client_id=534715734211821578&permissions=0&scope=bot`)
  message.author.send({embed});
...
client.on("message", message => {
 if (message.content === "-support") {
  const embed = new Discord.RichEmbed()
      .setColor("RANDOM")
      .setFooter('© .')  
      .addField('سيرفر الدعم الفني', `  Not Now  `)
  message.author.send({embed});
